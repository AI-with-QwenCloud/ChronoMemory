...
def _row_to_entry(row) -> MemoryEntry:
    (entry_id, text, embedding, importance, relevance_score, access_count,
...
def _semantic_recall(cur, query_embedding: list[float], top_k: int) -> dict[str, tuple[MemoryEntry, float]]:
    cur.execute(
        f"""
...
def _spreading_activation(cur, source_ids: list[str]) -> dict[str, tuple[MemoryEntry, float]]:
    if not source_ids:
        return {}
    ...
    cur.execute(
        ...
        FROM memories
        WHERE status = 'active' AND id = ANY(%s::uuid[])
        """,
        (target_ids,),
    )
    candidates: dict[str, tuple[MemoryEntry, float]] = {}
...
def recall(cur, query_text: str, top_k: int = 10) -> list[tuple[MemoryEntry, float]]:
    query_embedding = embed(query_text)

    semantic_candidates = _semantic_recall(cur, query_embedding, top_k)
    linked_candidates = _spreading_activation(cur, list(semantic_candidates.keys()))
