...
for provenance, expected_trust in TRUST_SCORES.items():
    entry = MemoryEntry(text=f"fact via {provenance}", embedding=DUMMY_EMBEDDING, provenance=provenance)
...
for provenance in EXPECTED_FLAGGED:
    entry = MemoryEntry(text="x", embedding=DUMMY_EMBEDDING, provenance=provenance)
...
try:
    MemoryEntry(text="x", embedding=DUMMY_EMBEDDING, provenance="not_a_real_source")
